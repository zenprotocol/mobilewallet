...
class PortfolioStore {
  ...
  getAssetName(asset) {
    ...
    const matchingActiveContract = this.activeContractsStore.activeContracts.find(
      ac => ac.contractId === contractId
    );
  ...
  get assets() {
    return this.rawAssets.map(asset => ({
      ...
      balance: isZenAsset(asset.asset)
        ? kalapasToZen(asset.balance)
        : asset.balance,
  ...
  filteredBalances = query => {
    ...
    return this.assets.filter(
      asset => asset.name.indexOf(query) > -1 || asset.asset.indexOf(query) > -1
    );
  };
